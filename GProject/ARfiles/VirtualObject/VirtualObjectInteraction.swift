...
//        sceneView.addGestureRecognizer(pinchGesture)
    ...
    @objc func didPinch(_ gesture : UIPinchGestureRecognizer){
        ...
        if let object = trackedObject,gesture.state == .changed {
                let scale = object.objectScale
                object.objectScale = SCNVector3.init(scale.x * Float(gesture.scale),
                                                       scale.y * Float(gesture.scale),
                                                       scale.z * Float(gesture.scale))
